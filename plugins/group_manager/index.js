...
export default {
    ...
    textMatchers: [
        {
            ...
            extractArgs: (match, message, text) => {
                ...
                const targetJids = mentionedJids.filter(jid => {
     ...
     * Ajoute un filtre
     */
    async _addFilter(groupJid, args, sender) {
        try {
     ...
     * Supprime un filtre
     */
    async _removeFilter(groupJid, filterId) {
        try {
     ...
     */
    async _addWhitelist(groupJid, args, sender, message) {
     ...
     * Configure le groupe
     */
    async _configure(groupJid, args) {
        try {
            switch (args.action) {
                case 'enable':
     ...
     */
    async _resetWarnings(groupJid, args, message) {
        let targetJid = args.user_jid;
        const mentions = message.mentionedJids || [];
     ...
     */
    async _banUser(chatId, args, message, transport) {
        ...
        if (!targetJid && msgMentions.length > 0) {
            ...
            const nonBotMentions = msgMentions.filter(jid => {
     ...
     */
    async _generateInvite(chatId, transport) {
        try {
     ...
     */
    async _muteUser(chatId, args, message) {
        let targetJid = args.user_jid;
        const mentions = message.mentionedJids || [];
     ...
     */
    _getUsername(rawMessage, targetJid) {
     ...
     */
    async _setMission(chatId, args, sender) {
     ...
     */
    async _analyzeAndExecuteMission(chatId, missionText, context) {
    ...
    async _listMyGroups(userJid) {
        try {
            ...
            const groupMap = new Map((groupData || []).map(g => [g.jid, g.name]));

            const list = groups.map((jid, i) => {
     ...
     */
    async _unmuteUser(groupJid, args, message) {
        try {
     ...
     */
    async _promoteUser(groupJid, args, message, transport) {
        try {
            let targetJid = args.user_jid;
            if (!targetJid && message.quoted?.sender) {
                targetJid = message.quoted.sender;
     ...
     */
    async _demoteUser(groupJid, args, message, transport) {
        try {
            let targetJid = args.user_jid;
            if (!targetJid && message.quoted?.sender) {
                targetJid = message.quoted.sender;
     ...
     */
    async _getGroupInfo(groupJid, transport) {
        try {
     ...
     */
    async _kickUser(chatId, args, message, transport) {
     ...
     */
    async _warnUser(chatId, args, message) {
        try {
            let targetJid = args.user_jid;
     ...
     */
    async _lockGroup(chatId, args, transport) {
        try {
