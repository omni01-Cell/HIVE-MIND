...
export class BotCore {
    ...
    async init() {
        ...
        // 3. Initialiser le scheduler (désactivé en mode local/CLI pour préserver les quotas de test)
        ...
        if (!isLocalOrCli) {
            ...
            console.log('[Core] 🛡️ Mode LOCAL/CLI détecté : désactivation du Scheduler, WakeSystem, MailboxWatcher et FeedbackService pour économiser le budget API.');
        }
...
- STRICTLY DO NOT claim success under any circumstances.
                                ...
                                if (!finalResponse || typeof finalResponse !== 'string' || finalResponse.trim() === '') {
                                    throw new Error('Empty summary response content');
                                }
