...
function parseDefinitionsRegexFallback(absolutePath: string): SymbolDefinition[] | null {
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // Track class scope brace count
            if (trimmed.includes('{')) braceScope++;
            if (trimmed.includes('}')) braceScope--;
            ...
            if (classMatch) {
                ...
                braceScope = trimmed.includes('{') ? 1 : 0;
                definitions.push({
                    name: currentClass,
                    kind: 'class',
                    startLine: i,
                    endLine: i, // signature level
                    lineCount: 1,
                    signatureLine: line,
            ...
            // Reset class scope if out of class braces
            if (currentClass && braceScope <= 0 && trimmed.includes('}')) {
                currentClass = undefined;
            }

            // Match function definition
            const funcMatch = trimmed.match(/^(?:export\s+)?(?:default\s+)?function\s+(\w+)\s*\(/);
            if (funcMatch) {
                definitions.push({
                    name: funcMatch[1],
                    kind: 'function',
            ...
            // Match method definition inside class
            if (currentClass) {
        ...
        try {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (regex.test(lineText)) {
                    // Check if it's a definition (contains class, function, or method keyword)
                    const isDefinition = /\bclass\b|\bfunction\b|\bpublic\b|\bprivate\b|\bmethod\b/.test(lineText) || lineText.includes(`${symbolName}(`);
