...
export const plugin = {
     ...
     */
    async execute(args: unknown, context: VisualReporterContext, toolName: string) {
        ...
        if (toolName === 'generate_pdf_report') {
            return await this.generatePdfReport(args as GeneratePdfArgs, transport, chatId);
        }
    ...
    async generatePdfReport({ title, content, filename, sections }: GeneratePdfArgs, transport: VisualReporterTransport, chatId: string) {
        try {
            ...
            // Header
            const botName = botIdentity.fullName;
            doc.fontSize(20).text(title, { align: 'center', underline: true });
            doc.moveDown();
            doc.fontSize(10).text(`Generated by ${botName} (WhatsApp Bot) - ${new Date().toLocaleString()}`, { align: 'right', italic: true });
            doc.moveDown(2);
            ...
            console.error('[VisualReporter] PDF generation error:', err);
            return {
                success: false,
                error: err.message
            };
