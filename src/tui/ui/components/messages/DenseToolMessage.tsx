...
interface DenseToolMessageViewProps {
    name: string;
    status: CoreToolCallStatus;
    description: React.ReactNode;
    summary: React.ReactNode;
    ...
    diffLines: React.ReactNode[];
    viewParts: ViewParts;
    outputFile: string | undefined;
    terminalWidth: number;
    isFocused: boolean;
    toggleRef: React.RefObject<DOMElement>;
}

function DenseToolMessageView({ name, status, description, summary, showPayload, isAlternateBuffer, diff, diffLines, viewParts, outputFile, terminalWidth, isFocused, toggleRef }: DenseToolMessageViewProps) {
    const keyExtractor = (_item: React.ReactNode, index: number) => `diff-line-${index}`;
    const renderItem = ({ item }: { item: React.ReactNode }) => <Box minHeight={1}>{item}</Box>;
    return (
            ...
            <Box marginLeft={2} flexDirection="row" flexWrap="wrap">
                <Box flexDirection="row" flexShrink={1}>
                    ...
                    <Box maxWidth={25} flexShrink={0} flexGrow={0}>
                        <Text color={theme.text.primary} bold wrap="truncate-end">{name}{' '}</Text>
                    </Box>
                    <Box marginLeft={1} flexShrink={1} flexGrow={0}>{description}</Box>
                </Box>
                {summary && (
                    <Box key="tool-summary" ref={isAlternateBuffer && diff ? toggleRef : undefined} marginLeft={1} flexGrow={0}>
                        {summary}
                    </Box>
                )}
            </Box>
            {showPayload && isAlternateBuffer && diffLines.length > 0 && (
                <Box marginLeft={PAYLOAD_MARGIN_LEFT} marginTop={1} marginBottom={1} paddingX={1} flexDirection="column"
                    height={Math.min(diffLines.length, COMPACT_TOOL_SUBVIEW_MAX_LINES) + 2}
                    maxHeight={COMPACT_TOOL_SUBVIEW_MAX_LINES + 2}
                    borderStyle="round" borderColor={theme.border.default} borderDimColor={true}
                    maxWidth={Math.min(PAYLOAD_MAX_WIDTH, terminalWidth - PAYLOAD_MARGIN_LEFT)}
                >
                    <ScrollableList data={diffLines} renderItem={renderItem} keyExtractor={keyExtractor}
                        estimatedItemHeight={() => 1} hasFocus={isFocused}
                        width={Math.min(PAYLOAD_MAX_WIDTH, terminalWidth - PAYLOAD_MARGIN_LEFT - PAYLOAD_BORDER_CHROME_WIDTH - PAYLOAD_SCROLL_GUTTER)}
                    />
                </Box>
            )}
            {showPayload && (!isAlternateBuffer || !diff) && viewParts.payload && (
                <Box marginLeft={PAYLOAD_MARGIN_LEFT} marginTop={1} marginBottom={1}>{viewParts.payload}</Box>
            )}
...
export const DenseToolMessage: React.FC<DenseToolMessageProps> = (props) => {
    ...
    return <DenseToolMessageView name={name} status={status} description={description} summary={summary} showPayload={showPayload} isAlternateBuffer={isAlternateBuffer} diff={diff} diffLines={diffLines} viewParts={viewParts} outputFile={outputFile} terminalWidth={terminalWidth} isFocused={isFocused} toggleRef={toggleRef} />;
};
