...
const renderTabsFn = (
    ...
    if (shellList.length > tabs.length && !isListOpenProp) {
        const overflowLabel = ` ... (${toggleBgShellListCmd}) `;
...
const renderProcessListFn = (
    ...
    setIsBackgroundTaskListOpen: (open: boolean) => void,
    setHighlightedPid: (pid: number | null) => void
) => {
...
const renderOutputFn = (
    output: string | AnsiOutput,
    isFocused: boolean,
    outputRef: RefObject<ScrollableListRef<AnsiLine | string> | null>
) => {
...
export const BackgroundTaskDisplay = ({
    ...
    const renderTabs = () =>
        renderTabsFn(
            shells, activePid, width, isFocused, isListOpenProp,
            helpTextStr, formatCommand(Command.TOGGLE_BACKGROUND_SHELL_LIST)
        );

    const renderProcessList = () =>
        renderProcessListFn(
            shells, activePid, width, height, isFocused,
            setActiveBackgroundTaskPid, setIsBackgroundTaskListOpen, setHighlightedPid
        );
    ...
    const renderOutput = () => renderOutputFn(output, isFocused, outputRef);
