...
export const Scrollable: React.FC<ScrollableProps> = ({
    ...
    const id = useId();
    const [size, setSize] = useState({ innerHeight: typeof height === 'number' ? height : 0, scrollHeight: 0 });
    const sizeRef = useRef(size);
    const scrollTopRef = useRef(scrollTop);

    useLayoutEffect(() => { sizeRef.current = size; }, [size]);
    useLayoutEffect(() => { scrollTopRef.current = scrollTop; }, [scrollTop]);
    ...
    useEffect(() => () => { viewportObserverRef.current?.disconnect(); contentObserverRef.current?.disconnect(); }, []);
    ...
    useKeypress(
        (key: Key) => {
            ...
            const maxScroll = Math.max(0, scrollHeight - innerHeight);
            const actualScrollTop = Math.min(newScrollTop, maxScroll);
