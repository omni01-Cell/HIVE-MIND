...
function isAtWordEnd(chars: string[], i: number): boolean {
...
export const findWordEndInLine = (line: string, col: number): number | null => {
    ...
    if (isAtWordEnd(chars, i) || isAtPunctuationEnd(chars, i)) {
        i++;
...
export const replaceRangeInternal = (
    ...
    const clampVal = (value: number, min: number, max: number) =>
    ...
    const sCol = clampVal(startCol, 0, currentLineLen(startRow));
    const eCol = clampVal(endCol, 0, currentLineLen(endRow));
            ...
            while (currentPosInLogLine < codePointsInLogLine.length) {
                ...
                for (let i = currentPosInLogLine; i < codePointsInLogLine.length; i++) {
                    ...
                    if (currentChunkVisualWidth + charVisualWidth > viewportWidth) {
                        ...
                        const hasValidWordBreak =
                            lastWordBreakPoint !== -1 &&
                            numCodePointsAtLastWordBreak > 0 &&
                            currentPosInLogLine + numCodePointsAtLastWordBreak < i;

                        if (hasValidWordBreak) {
...
function handleInsertCase(
    ...
    const currentLineAt = (r: number) => newLines[r] ?? '';
    let insertPayload = payload;
    let newPastedContent = nextState.pastedContent;
    if (isPaste) {
        insertPayload = insertPayload.replace(/\r\n|\r/g, '\n');
        const lineCount = insertPayload.split('\n').length;
        if (
            lineCount > LARGE_PASTE_LINE_THRESHOLD ||
            insertPayload.length > LARGE_PASTE_CHAR_THRESHOLD
        ) {
            const id = generatePastedTextId(insertPayload, lineCount, newPastedContent);
            newPastedContent = { ...newPastedContent, [id]: insertPayload };
            insertPayload = id;
        }
    }
    ...
    if (options.inputFilter) {
        insertPayload = options.inputFilter(insertPayload);
    }
    if (insertPayload.length === 0) {
        return state;
    }
    const str = stripUnsafeCharacters(
        insertPayload.replace(/\r\n/g, '\n').replace(/\r/g, '\n')
    );
    const parts = str.split('\n');
    const lineContent = currentLineAt(newCursorRow);
    ...
    const { newInfo: newExpandedPaste, isDetached } = shiftExpandedRegions(
        nextState.expandedPaste, nextState.cursorRow, lineDelta
    );
...
function handleBackspaceCase(state: TextBufferState): TextBufferState {
    ...
    if (placeholder) {
        ...
        newLines[cursorRow] = cpSlice(newLines[cursorRow], 0, placeholder.start) + cpSlice(newLines[cursorRow], placeholder.end);
        const newTransformations = [...currentState.transformationsByLine];
        newTransformations[cursorRow] = calculateTransformationsForLine(newLines[cursorRow]);
        let newPastedContent = currentState.pastedContent;
        if (placeholder.type === 'paste' && placeholder.id) {
            const { [placeholder.id]: _, ...remaining } = currentState.pastedContent;
            newPastedContent = remaining;
        }
        return { ...currentState, lines: newLines, cursorCol: placeholder.start, preferredCol: null, transformationsByLine: newTransformations, pastedContent: newPastedContent };
    }
    const newLines = [...currentState.lines];
    let newCursorRow = currentState.cursorRow;
    let newCursorCol = currentState.cursorCol;
    const currentLineAt = (r: number) => newLines[r] ?? '';
    let lineDelta = 0;
    if (newCursorCol > 0) {
        const lineContent = currentLineAt(newCursorRow);
        ...
        const prevLineContent = currentLineAt(newCursorRow - 1);
        const currentLineContentVal = currentLineAt(newCursorRow);
    ...
    const { newInfo: newExpandedPaste, isDetached } = shiftExpandedRegions(
        currentState.expandedPaste, currentState.cursorRow + lineDelta, lineDelta, currentState.cursorRow
    );
    const newPastedContent = { ...currentState.pastedContent };
    if (isDetached && currentState.expandedPaste) {
        delete newPastedContent[currentState.expandedPaste.id];
    }
    return { ...currentState, lines: newLines, cursorRow: newCursorRow, cursorCol: newCursorCol, preferredCol: null, pastedContent: newPastedContent, expandedPaste: newExpandedPaste };
}
...
function applyVisualDirection(
    ...
    visualLines: string[],
    currentVisLineLen: number
): { row: number; col: number; preferredCol: number | null } | null {
    let row = initialRow;
    let col = initialCol;
    let preferredCol = initialPreferredCol;
    switch (dir) {
        case 'left':
            preferredCol = null;
            if (col > 0) { col--; } else if (row > 0) { row--; col = cpLen(visualLines[row] ?? ''); }
            break;
        case 'right':
            preferredCol = null;
            if (col < currentVisLineLen) { col++; } else if (row < visualLines.length - 1) { row++; col = 0; }
            break;
        case 'up':
            if (row > 0) { if (preferredCol === null) preferredCol = col; row--; col = clamp(preferredCol, 0, cpLen(visualLines[row] ?? '')); }
            break;
        case 'down':
            if (row < visualLines.length - 1) { if (preferredCol === null) preferredCol = col; row++; col = clamp(preferredCol, 0, cpLen(visualLines[row] ?? '')); }
            break;
        case 'home': preferredCol = null; col = 0; break;
        case 'end': preferredCol = null; col = currentVisLineLen; break;
        default: return null;
    }
    return { row, col, preferredCol };
}

function handleMoveCase(state: TextBufferState, dir: Direction): TextBufferState {
    ...
    if (dir === 'left' || dir === 'right' || dir === 'up' || dir === 'down' || dir === 'home' || dir === 'end') {
        ...
        const currentVisLineLen = cpLen(visualLines[visualCursor[0]] ?? '');
        const result = applyVisualDirection(dir, visualCursor[0], visualCursor[1], preferredCol, visualLines, currentVisLineLen);
        if (!result) return state;
        const { row: newVisualRow, col: newVisualCol, preferredCol: newPreferredCol } = result;
        if (visualToLogicalMap[newVisualRow]) {
            ...
            while (transformedStartCol < transformedToLogicalMap.length && transformedToLogicalMap[transformedStartCol] < logicalStartCol) { transformedStartCol++; }
            ...
            return { ...state, cursorRow: logRow, cursorCol: newLogicalCol, preferredCol: newPreferredCol };
        }
        return state;
    }
    switch (dir) {
        case 'wordLeft': {
            ...
            if (cursorCol === 0) { newCursorRow--; newCursorCol = cpLen(lines[newCursorRow] ?? ''); } else { const lineContent = lines[cursorRow]; newCursorCol = findPrevWordBoundary(lineContent, cursorCol); }
            return { ...state, cursorRow: newCursorRow, cursorCol: newCursorCol, preferredCol: null };
        }
        case 'wordRight': {
            ...
            if (cursorCol >= lineLen) { newCursorRow++; newCursorCol = 0; } else { newCursorCol = findNextWordBoundary(lineContent, cursorCol); }
            return { ...state, cursorRow: newCursorRow, cursorCol: newCursorCol, preferredCol: null };
        }
...
function handleDeleteCase(state: TextBufferState): TextBufferState {
    const stateWithUndo = pushUndo(state);
    const currentState = detachExpandedPaste(stateWithUndo);
    const { cursorRow, cursorCol, lines, transformationsByLine } = currentState;
    const transformations = transformationsByLine[cursorRow] ?? [];
    ...
    if (placeholder) {
        ...
        return { ...currentState, lines: newLines, preferredCol: null, transformationsByLine: newTransformations, pastedContent: newPastedContent };
    }
    const lineContent = state.lines[cursorRow] ?? '';
    let lineDelta = 0;
    const newLines = [...currentState.lines];
    if (cursorCol < cpLen(state.lines[cursorRow] ?? '')) {
        newLines[cursorRow] = cpSlice(lineContent, 0, cursorCol) + cpSlice(lineContent, cursorCol + 1);
    } else if (cursorRow < lines.length - 1) {
        const nextLineContent = state.lines[cursorRow + 1] ?? '';
    ...
    const { newInfo: newExpandedPaste, isDetached } = shiftExpandedRegions(
        currentState.expandedPaste, currentState.cursorRow, lineDelta, currentState.cursorRow + (lineDelta < 0 ? 1 : 0)
    );
    const newPastedContent = { ...currentState.pastedContent };
    if (isDetached && currentState.expandedPaste) {
        delete newPastedContent[currentState.expandedPaste.id];
    }
    return { ...currentState, lines: newLines, preferredCol: null, pastedContent: newPastedContent, expandedPaste: newExpandedPaste };
}

function handleDeleteWordLeftCase(state: TextBufferState): TextBufferState {
    const stateWithUndo = pushUndo(state);
    const currentState = detachExpandedPaste(stateWithUndo);
    ...
    if (newCursorCol > 0) {
        const lineContent = state.lines[newCursorRow] ?? '';
        ...
        const prevLineContent = state.lines[cursorRow - 1] ?? '';
        const currentLineContentVal = state.lines[cursorRow] ?? '';
    ...
    const newPastedContent = pruneOrphanedPastedContent(currentState.pastedContent, currentState.expandedPaste?.id ?? null, beforeChangedLines, newLines);
    return { ...currentState, lines: newLines, cursorRow: newCursorRow, cursorCol: newCursorCol, preferredCol: null, pastedContent: newPastedContent };
}

function handleDeleteWordRightCase(state: TextBufferState): TextBufferState {
    const stateWithUndo = pushUndo(state);
    const currentState = detachExpandedPaste(stateWithUndo);
    const { cursorRow, cursorCol, lines } = currentState;
    const lineContent = state.lines[cursorRow] ?? '';
    const lineLen = cpLen(lineContent);
    if (cursorCol >= lineLen && cursorRow === lines.length - 1) { return currentState; }
    const newLines = [...currentState.lines];
    let beforeChangedLines: string[] = [];
    ...
    const newPastedContent = pruneOrphanedPastedContent(currentState.pastedContent, currentState.expandedPaste?.id ?? null, beforeChangedLines, newLines);
    return { ...currentState, lines: newLines, preferredCol: null, pastedContent: newPastedContent };
}

function handleKillLineRightCase(state: TextBufferState): TextBufferState {
    const stateWithUndo = pushUndo(state);
    const currentState = detachExpandedPaste(stateWithUndo);
    const { cursorRow, cursorCol, lines } = currentState;
    const lineContent = state.lines[cursorRow] ?? '';
    if (cursorCol < cpLen(lineContent)) {
        ...
        const newPastedContent = pruneOrphanedPastedContent(currentState.pastedContent, currentState.expandedPaste?.id ?? null, [lineContent], newLines);
        return { ...currentState, lines: newLines, preferredCol: null, pastedContent: newPastedContent };
    } else if (cursorRow < lines.length - 1) {
        const nextLineContent = state.lines[cursorRow + 1] ?? '';
        const newLines = [...currentState.lines];
        newLines[cursorRow] = lineContent + nextLineContent;
        newLines.splice(cursorRow + 1, 1);
        const newPastedContent = pruneOrphanedPastedContent(currentState.pastedContent, currentState.expandedPaste?.id ?? null, [lineContent, nextLineContent], newLines);
        return { ...currentState, lines: newLines, preferredCol: null, pastedContent: newPastedContent };
    }
    return currentState;
}

function handleKillLineLeftCase(state: TextBufferState): TextBufferState {
    const stateWithUndo = pushUndo(state);
    const currentState = detachExpandedPaste(stateWithUndo);
    const { cursorRow, cursorCol } = currentState;
    if (cursorCol > 0) {
        ...
        const newPastedContent = pruneOrphanedPastedContent(currentState.pastedContent, currentState.expandedPaste?.id ?? null, [lineContent], newLines);
        return { ...currentState, lines: newLines, cursorCol: 0, preferredCol: null, pastedContent: newPastedContent };
    }
    return currentState;
}

function handleTogglePasteExpansionCase(
    state: TextBufferState,
    id: string,
    row: number,
    col: number
    ...
    if (expandedPaste && expandedPaste.id === id) {
        ...
        return { ...nextState, lines: newLines, cursorRow: newCursorRow, cursorCol: newCursorCol, preferredCol: null, expandedPaste: null };
    }
    ...
    if (state.expandedPaste) {
        ...
        if (targetRow !== undefined && targetRow > existingInfo.startLine) { targetRow += lineDelta; }
        currentState = textBufferReducerLogic(state, { type: 'toggle_paste_expansion', payload: { id: existingInfo.id, row: existingInfo.startLine, col: 0 } });
    ...
    const tryFindOnLine = (idx: number) => {
        ...
        let transform = transforms.find((t) => t.type === 'paste' && t.id === id && col >= t.logStart && col <= t.logEnd);
        if (!transform) { transform = transforms.find((t) => t.type === 'paste' && t.id === id); }
        if (transform) { lineIndex = idx; placeholderStart = transform.logStart; return true; }
        return false;
    };
    if (targetRow >= 0 && targetRow < currentState.lines.length) { tryFindOnLine(targetRow); }
    if (lineIndex === -1) { for (let i = 0; i < currentState.lines.length; i++) { if (tryFindOnLine(i)) break; } }
    if (lineIndex === -1) return currentState;
    const nextState = pushUndo(currentState);
    ...
    if (contentLines.length === 1) {
        ...
        expandedLines = [prefix + contentLines[0], ...contentLines.slice(1, -1), contentLines[contentLines.length - 1] + suffix];
    }
    ...
    return { ...nextState, lines: newLines, cursorRow: newCursorRow, cursorCol: newCursorCol, preferredCol: null, expandedPaste: { id, startLine: lineIndex, lineCount: expandedLines.length, prefix, suffix } };
}

function handleSetTextCase(
    state: TextBufferState,
    payload: string,
    pushToUndo: boolean | undefined,
    cursorPosition: 'start' | 'end' | number | undefined
): TextBufferState {
    let nextState = state;
    if (pushToUndo !== false) { nextState = pushUndo(state); }
    const newContentLines = payload.replace(/\r\n?/g, '\n').split('\n');
    const lines = newContentLines.length === 0 ? [''] : newContentLines;
    let newCursorRow: number;
    let newCursorCol: number;
    if (typeof cursorPosition === 'number') {
        [newCursorRow, newCursorCol] = offsetToLogicalPos(payload, cursorPosition);
    ...
    return { ...nextState, lines, cursorRow: newCursorRow, cursorCol: newCursorCol, preferredCol: null, pastedContent: payload === '' ? {} : nextState.pastedContent };
}

function handleReplaceRangeCase(
    state: TextBufferState,
    startRow: number,
    startCol: number,
    endRow: number,
    ...
    if (isDetached && nextState.expandedPaste) { delete newPastedContent[nextState.expandedPaste.id]; }
    return { ...newState, pastedContent: newPastedContent, expandedPaste: newExpandedPaste };
}
...
function textBufferReducerLogic(
    ...
    switch (action.type) {
        case 'set_text':
            return handleSetTextCase(state, action.payload, action.pushToUndo, action.cursorPosition);

        case 'insert':
            return handleInsertCase(state, action.payload, action.isPaste, options);
        ...
        case 'backspace':
            return handleBackspaceCase(state);
        ...
        case 'move':
            return handleMoveCase(state, action.payload.dir);
        ...
        case 'delete':
            return handleDeleteCase(state);

        case 'delete_word_left':
            return handleDeleteWordLeftCase(state);

        case 'delete_word_right':
            return handleDeleteWordRightCase(state);

        case 'kill_line_right':
            return handleKillLineRightCase(state);

        case 'kill_line_left':
            return handleKillLineLeftCase(state);
        ...
        case 'replace_range':
            return handleReplaceRangeCase(state, action.payload.startRow, action.payload.startCol, action.payload.endRow, action.payload.endCol, action.payload.text);
        ...
        case 'toggle_paste_expansion':
            return handleTogglePasteExpansionCase(state, action.payload.id, action.payload.row, action.payload.col);

        default: {
            if ((action.type as string).startsWith('vim_')) {
...
function tryHandleBoundedInputCommand(
    key: Key,
...
function useVimDeleteActions(dispatch: React.Dispatch<TextBufferAction>) {
    ...
    const vimFindCharForward = useCallback(
        (char: string, count: number, till: boolean): void => {
            dispatch({ type: 'vim_find_char_forward', payload: { char, count, till } });
        },
        [dispatch]
    );
    ...
    const vimFindCharBackward = useCallback(
        (char: string, count: number, till: boolean): void => {
            dispatch({ type: 'vim_find_char_backward', payload: { char, count, till } });
        },
        [dispatch]
    );
    ...
    const vimDeleteToCharForward = useCallback(
        (char: string, count: number, till: boolean): void => {
            dispatch({ type: 'vim_delete_to_char_forward', payload: { char, count, till } });
        },
        [dispatch]
    );
    ...
    const vimDeleteToCharBackward = useCallback(
        (char: string, count: number, till: boolean): void => {
            dispatch({ type: 'vim_delete_to_char_backward', payload: { char, count, till } });
        },
        [dispatch]
    );
...
function useVimNavigationActions(dispatch: React.Dispatch<TextBufferAction>) {
...
interface BasicActionsDeps {
    dispatch: React.Dispatch<TextBufferAction>;
    singleLine: boolean;
...
function useBasicActions(d: BasicActionsDeps) {
    const insert = useCallback(
        (ch: string, { paste = false }: { paste?: boolean } = {}): void => {
            if (typeof ch !== 'string') return;
            let textToInsert = ch;
            const minLengthToInferAsDragDrop = 3;
            if (ch.length >= minLengthToInferAsDragDrop && !d.shellModeActive && paste && d.escapePastedPaths) {
                const processed = parsePastedPaths(ch.trim());
                if (processed) textToInsert = processed;
            }
            let currentText = '';
            for (const char of toCodePoints(textToInsert)) {
                if (char.codePointAt(0) === 127) {
                    if (currentText.length > 0) { d.dispatch({ type: 'insert', payload: currentText, isPaste: paste }); currentText = ''; }
                    d.dispatch({ type: 'backspace' });
                } else { currentText += char; }
            }
            if (currentText.length > 0) d.dispatch({ type: 'insert', payload: currentText, isPaste: paste });
        },
        [d.shellModeActive, d.escapePastedPaths, d.dispatch]
    );
    const newline = useCallback((): void => { if (!d.singleLine) d.dispatch({ type: 'insert', payload: '\n' }); }, [d.singleLine, d.dispatch]);
    const backspace = useCallback((): void => { d.dispatch({ type: 'backspace' }); }, [d.dispatch]);
    const del = useCallback((): void => { d.dispatch({ type: 'delete' }); }, [d.dispatch]);
    const move = useCallback((dir: Direction): void => { d.dispatch({ type: 'move', payload: { dir } }); }, [d.dispatch]);
    const undo = useCallback((): void => { d.dispatch({ type: 'undo' }); }, [d.dispatch]);
    const redo = useCallback((): void => { d.dispatch({ type: 'redo' }); }, [d.dispatch]);
    const setText = useCallback((newText: string, cursorPosition?: 'start' | 'end' | number): void => { d.dispatch({ type: 'set_text', payload: newText, cursorPosition }); }, [d.dispatch]);
    const deleteWordLeft = useCallback((): void => { d.dispatch({ type: 'delete_word_left' }); }, [d.dispatch]);
    const deleteWordRight = useCallback((): void => { d.dispatch({ type: 'delete_word_right' }); }, [d.dispatch]);
    const killLineRight = useCallback((): void => { d.dispatch({ type: 'kill_line_right' }); }, [d.dispatch]);
    const killLineLeft = useCallback((): void => { d.dispatch({ type: 'kill_line_left' }); }, [d.dispatch]);
    const replaceRange = useCallback((startRow: number, startCol: number, endRow: number, endCol: number, replacementText: string): void => { d.dispatch({ type: 'replace_range', payload: { startRow, startCol, endRow, endCol, text: replacementText } }); }, [d.dispatch]);
    const replaceRangeByOffset = useCallback((startOffset: number, endOffset: number, replacementText: string): void => { const [sR, sC] = offsetToLogicalPos(d.text, startOffset); const [eR, eC] = offsetToLogicalPos(d.text, endOffset); replaceRange(sR, sC, eR, eC, replacementText); }, [d.text, replaceRange]);
    const moveToOffset = useCallback((offset: number): void => { d.dispatch({ type: 'move_to_offset', payload: { offset } }); }, [d.dispatch]);
    const getOffset = useCallback((): number => logicalPosToOffset(d.lines, d.cursorRow, d.cursorCol), [d.lines, d.cursorRow, d.cursorCol]);
    const togglePasteExpansion = useCallback((id: string, row: number, col: number): void => { d.dispatch({ type: 'toggle_paste_expansion', payload: { id, row, col } }); }, [d.dispatch]);
    const getExpandedPasteAtLineCallback = useCallback((lineIndex: number): string | null => getExpandedPasteAtLine(lineIndex, d.expandedPaste), [d.expandedPaste]);
    return { insert, newline, backspace, del, move, undo, redo, setText, deleteWordLeft, deleteWordRight, killLineRight, killLineLeft, replaceRange, replaceRangeByOffset, moveToOffset, getOffset, togglePasteExpansion, getExpandedPasteAtLineCallback };
}
...
function useInputHandler(d: InputHandlerDeps): (key: Key) => boolean {
    return useCallback(
        (key: Key): boolean => {
            const { sequence: input } = key;
            const boundedResult = tryHandleBoundedInputCommand(key, d.keyMatchers, d.lines, d.cursorRow, d.cursorCol, d.undoStackLength, d.redoStackLength, d.move, d.del, d.undo, d.redo);
            if (boundedResult !== null) return boundedResult;
            if (key.name === 'paste') { d.insert(input, { paste: true }); return true; }
            if (d.keyMatchers[Command.RETURN](key) || d.keyMatchers[Command.NEWLINE](key)) { if (d.singleLine) return false; d.newline(); return true; }
            if (d.keyMatchers[Command.MOVE_LEFT](key)) { if (d.cursorRow === 0 && d.cursorCol === 0) return false; d.move('left'); return true; }
            if (d.keyMatchers[Command.MOVE_UP](key)) { if (d.visualCursor[0] === 0) return false; d.move('up'); return true; }
            if (d.keyMatchers[Command.MOVE_DOWN](key)) { if (d.visualCursor[0] === d.visualLines.length - 1) return false; d.move('down'); return true; }
            if (d.keyMatchers[Command.MOVE_WORD_LEFT](key)) { d.move('wordLeft'); return true; }
            if (d.keyMatchers[Command.MOVE_WORD_RIGHT](key)) { d.move('wordRight'); return true; }
            if (d.keyMatchers[Command.HOME](key)) { d.move('home'); return true; }
            if (d.keyMatchers[Command.END](key)) { d.move('end'); return true; }
            if (d.keyMatchers[Command.CLEAR_INPUT](key)) { if (d.text.length > 0) { d.setText(''); return true; } return false; }
            if (d.keyMatchers[Command.DELETE_WORD_BACKWARD](key)) { d.deleteWordLeft(); return true; }
            if (d.keyMatchers[Command.DELETE_WORD_FORWARD](key)) { d.deleteWordRight(); return true; }
            if (d.keyMatchers[Command.DELETE_CHAR_LEFT](key)) { d.backspace(); return true; }
            if (key.insertable) { d.insert(input, { paste: false }); return true; }
            return false;
        },
        [d.keyMatchers, d.lines, d.cursorRow, d.cursorCol, d.singleLine, d.text, d.visualCursor, d.visualLines, d.undoStackLength, d.redoStackLength, d.insert, d.newline, d.backspace, d.del, d.move, d.undo, d.redo, d.setText, d.deleteWordLeft, d.deleteWordRight]
    );
}

interface VisualPositioningDeps {
    visualLayout: VisualLayout;
    lines: string[];
    dispatch: React.Dispatch<TextBufferAction>;
}

function useVisualPositioning(d: VisualPositioningDeps) {
    const moveToVisualPosition = useCallback(
        (visRow: number, visCol: number): void => {
            const { visualLines: mvVisualLines, visualToLogicalMap: mvVisualToLogicalMap, transformedToLogicalMaps: mvTransformedToLogicalMaps, visualToTransformedMap: mvVisualToTransformedMap } = d.visualLayout;
            const clampedVisRow = Math.max(0, Math.min(visRow, mvVisualLines.length - 1));
            const visualLine = mvVisualLines[clampedVisRow] || '';
            if (mvVisualToLogicalMap[clampedVisRow]) {
                const [logRow] = mvVisualToLogicalMap[clampedVisRow];
                const transformedToLogicalMap = mvTransformedToLogicalMaps?.[logRow] ?? [];
                const startColInTransformed = mvVisualToTransformedMap?.[clampedVisRow] ?? 0;
                const codePoints = toCodePoints(visualLine);
                let currentVisX = 0;
                let charOffset = 0;
                for (const char of codePoints) {
                    const charWidth = getCachedStringWidth(char);
                    if (visCol < currentVisX + charWidth) { if (charWidth > 1 && visCol >= currentVisX + charWidth / 2) charOffset++; break; }
                ...
                const newCursorCol = transformedToLogicalMap[transformedCol] ?? cpLen(d.lines[logRow] ?? '');
                d.dispatch({ type: 'set_cursor', payload: { cursorRow: logRow, cursorCol: newCursorCol, preferredCol: charOffset } });
            }
        },
        [d.visualLayout, d.lines, d.dispatch]
    );
    const getLogicalPositionFromVisual = useCallback(
        (visRow: number, visCol: number): { row: number; col: number } | null => {
            const { visualLines: glpvVisualLines, visualToLogicalMap: glpvVisualToLogicalMap, transformedToLogicalMaps: glpvTransformedToLogicalMaps, visualToTransformedMap: glpvVisualToTransformedMap } = d.visualLayout;
            const clampedVisRow = Math.max(0, Math.min(visRow, glpvVisualLines.length - 1));
            const visualLine = glpvVisualLines[clampedVisRow] || '';
            if (!glpvVisualToLogicalMap[clampedVisRow]) return null;
            const [logRow] = glpvVisualToLogicalMap[clampedVisRow];
            const transformedToLogicalMap = glpvTransformedToLogicalMaps?.[logRow] ?? [];
            const startColInTransformed = glpvVisualToTransformedMap?.[clampedVisRow] ?? 0;
            const codePoints = toCodePoints(visualLine);
            let currentVisX = 0;
            let charOffset = 0;
            for (const char of codePoints) {
            ...
            charOffset = Math.min(charOffset, codePoints.length);
            const transformedCol = Math.min(startColInTransformed + charOffset, Math.max(0, transformedToLogicalMap.length - 1));
            return { row: logRow, col: transformedToLogicalMap[transformedCol] ?? cpLen(d.lines[logRow] ?? '') };
        },
        [d.visualLayout, d.lines]
    );
    return { moveToVisualPosition, getLogicalPositionFromVisual };
}
...
interface ExternalEditorDeps {
    ...
    getPreferredEditor?: () => EditorType | undefined;
    openEditorInNewWindow: boolean;
    dispatch: React.Dispatch<TextBufferAction>;
}

function useExternalEditor(d: ExternalEditorDeps): () => Promise<void> {
    return useCallback(async (): Promise<void> => {
        const tmpDir = fs.mkdtempSync(pathMod.join(os.tmpdir(), 'gemini-edit-'));
        const filePath = pathMod.join(tmpDir, 'buffer.txt');
        const expandedText = expandPastePlaceholders(d.text, d.pastedContent);
        fs.writeFileSync(filePath, expandedText, 'utf8');
        d.dispatch({ type: 'create_undo_snapshot' });
        try {
            await openFileInEditor(filePath, d.stdin, d.setRawMode, d.getPreferredEditor?.(), d.openEditorInNewWindow);
            let newText = fs.readFileSync(filePath, 'utf8');
            newText = newText.replace(/\r\n?/g, '\n');
            const sortedPlaceholders = Object.entries(d.pastedContent).sort((a, b) => b[1].length - a[1].length);
            for (const [id, content] of sortedPlaceholders) { if (newText.includes(content)) newText = newText.replace(content, id); }
            d.dispatch({ type: 'set_text', payload: newText, pushToUndo: false });
        } catch (err) {
    ...
    }, [d.text, d.pastedContent, d.stdin, d.setRawMode, d.getPreferredEditor, d.openEditorInNewWindow, d.dispatch]);
}
...
export function useTextBuffer({
    ...
    const {
        lines, cursorRow, cursorCol, preferredCol, selectionAnchor,
        visualLayout, transformationsByLine, pastedContent, expandedPaste,
        undoStack, redoStack
    } = state;
    ...
    if (visualCursor[0] < scrollRowState) { newVisualScrollRow = visualCursor[0]; }
    else if (visualCursor[0] >= scrollRowState + height) { newVisualScrollRow = visualCursor[0] - height + 1; }
    newVisualScrollRow = clamp(newVisualScrollRow, 0, maxScrollStart);
    if (newVisualScrollRow !== scrollRowState) setScrollRowState(newVisualScrollRow);
    const actualScrollRowState = newVisualScrollRow;

    useEffect(() => { if (onChange) onChange(text); }, [text, onChange]);
    useEffect(() => { dispatch({ type: 'set_viewport', payload: { width: viewport.width, height: viewport.height } }); }, [viewport.width, viewport.height]);

    const basicActions = useBasicActions({ dispatch, singleLine, shellModeActive, escapePastedPaths, expandedPaste, text, lines, cursorRow, cursorCol });
    const vimDeleteActions = useVimDeleteActions(dispatch);
    const vimNavigationActions = useVimNavigationActions(dispatch);
    const openInExternalEditor = useExternalEditor({ text, pastedContent, stdin, setRawMode, getPreferredEditor, openEditorInNewWindow: settings.merged.general.openEditorInNewWindow, dispatch });
    const handleInput = useInputHandler({ keyMatchers, lines, cursorRow, cursorCol, singleLine, text, visualCursor, visualLines, undoStackLength: undoStack.length, redoStackLength: redoStack.length, ...basicActions });
    const { moveToVisualPosition, getLogicalPositionFromVisual } = useVisualPositioning({ visualLayout, lines, dispatch });

    const visualScrollRow = useMemo(() => Math.min(actualScrollRowState, Math.max(0, visualLines.length - viewport.height)), [visualLines.length, actualScrollRowState, viewport.height]);
    ...
    const returnValue: TextBuffer = useMemo(() => ({
        lines, text, cursor: [cursorRow, cursorCol], preferredCol, selectionAnchor, pastedContent,
        allVisualLines: visualLines, viewportVisualLines: renderedVisualLines, visualCursor, visualScrollRow,
        viewportHeight: viewport.height, visualToLogicalMap, transformedToLogicalMaps, visualToTransformedMap,
        transformationsByLine, visualLayout, ...basicActions, moveToVisualPosition, getLogicalPositionFromVisual,
        getExpandedPasteAtLine: basicActions.getExpandedPasteAtLineCallback, togglePasteExpansion: basicActions.togglePasteExpansion,
        expandedPaste, handleInput, openInExternalEditor, ...vimDeleteActions, ...vimNavigationActions
    }), [
        lines, text, cursorRow, cursorCol, preferredCol, selectionAnchor, pastedContent,
        visualLines, renderedVisualLines, visualCursor, visualScrollRow, viewport.height,
        visualToLogicalMap, transformedToLogicalMaps, visualToTransformedMap, transformationsByLine,
        visualLayout, basicActions, moveToVisualPosition, getLogicalPositionFromVisual,
        expandedPaste, handleInput, openInExternalEditor, vimDeleteActions, vimNavigationActions
    ]);
    return returnValue;
}
