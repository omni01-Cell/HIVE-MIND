...
function ExtensionDetailsContent({ extension, isInstalled, isLinkable, updateState }: ExtensionDetailsContentProps) {
    return (
        <Box flexDirection="column" paddingX={1} paddingY={0} height="100%" borderStyle="round" borderColor={theme.border.default}>
            {/* Header Row */}
            <Box flexDirection="row" justifyContent="space-between" marginBottom={1}>
                <Box>
                    <Text color={theme.text.secondary}>{'>'} Extensions {'>'}{' '}</Text>
                    <Text color={theme.text.primary} bold>{extension.extensionName}</Text>
                    {updateState === ExtensionUpdateState.UPDATE_AVAILABLE && (
                        <Box marginLeft={1}><Text color={theme.status.warning}>[I] Update</Text></Box>
                    )}
                    {updateState === ExtensionUpdateState.UPDATING && (
                        <Box marginLeft={1}><Text color={theme.text.secondary}>[Updating...]</Text></Box>
                    )}
                </Box>
                <Box flexDirection="row">
                    <Text color={theme.text.secondary}>{extension.extensionVersion ? `v${extension.extensionVersion}` : ''}{' '} |{' '}</Text>
            ...
            {/* Description */}
            <Box marginBottom={1}>
                <Text color={theme.text.primary}>{extension.extensionDescription || extension.repoDescription}</Text>
            </Box>
            ...
            <Box flexDirection="row" marginBottom={1}>
                {[
                    ...
                    extension.hasContext && { label: 'Context file', color: theme.status.error },
                    extension.hasHooks && { label: 'Hooks', color: theme.status.warning },
                    extension.hasSkills && { label: 'Skills', color: theme.status.success },
                    extension.hasCustomCommands && { label: 'Commands', color: theme.text.primary }
                ].filter((f): f is { label: string; color: string } => !!f).map((feature, index, array) => (
                    <Box key={feature.label} flexDirection="row">
                        <Text color={feature.color}>{feature.label} </Text>
                        {index < array.length - 1 && <Box marginRight={1}><Text color={theme.text.secondary}>|</Text></Box>}
                    </Box>
                ))}
            </Box>
            ...
            {extension.hasMCP && (
                <Box flexDirection="column" marginBottom={1}>
                    <Text color={theme.text.primary}>This extension will run the following MCP servers:</Text>
                    <Box marginLeft={2}><Text color={theme.text.primary}>* {extension.extensionName} (local)</Text></Box>
                </Box>
            )}

            {extension.hasContext && (
                <Box flexDirection="column" marginBottom={1}>
                    <Text color={theme.text.primary}>This extension will append info to your gemini.md context using gemini.md</Text>
                </Box>
            )}
            ...
            {!isInstalled && (
                <Box flexDirection="column" borderStyle="round" borderColor={theme.status.warning} paddingX={1} paddingY={0}>
                    ...
                    <Box marginTop={1} flexDirection="row">
                        <Box marginRight={2}><Text color={theme.text.primary}>[{'Enter'}] Install</Text></Box>
...
export function ExtensionDetails({
    ...
    if (isInstalling) {
        return (
            <Box flexDirection="column" paddingX={1} paddingY={0} height="100%" borderStyle="round" borderColor={theme.border.default} justifyContent="center" alignItems="center">
                <Text color={theme.text.primary}>Installing {extension.extensionName}...</Text>
            </Box>
        );
    }

    return <ExtensionDetailsContent extension={extension} isInstalled={isInstalled} isLinkable={isLinkable} updateState={updateState} />;
}
