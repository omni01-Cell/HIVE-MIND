...
function computeChoiceLayout(
    question: Question,
    selectionItems: Array<SelectionListItem<OptionItem>>,
    progressHeader: React.ReactNode,
    ...
    return { TITLE_MARGIN, questionHeightLimit, maxItemsToShow };
}

function renderChoiceItem(
    ...
    state: {
        ...
        question: Question;
        customBuffer: ReturnType<typeof useTextBuffer>;
        buildAnswerString: (indices: Set<number>, includeCustom: boolean, custom: string) => string;
        onAnswer: (answer: string) => void;
    }
): React.ReactNode {
    const { selectedIndices, isCustomOptionSelected, questionOptions, question, customBuffer, buildAnswerString, onAnswer } = state;

    const isChecked =
        (optionItem.type === 'option' && selectedIndices.has(optionItem.index)) ||
        (optionItem.type === 'other' && isCustomOptionSelected) ||
        (optionItem.type === 'all' && selectedIndices.size === questionOptions.length);
    const showCheck =
        question.multiSelect &&
        (optionItem.type === 'option' || optionItem.type === 'other' || optionItem.type === 'all');
                    ...
                    <ClickableCheckbox
                        ...
                        onClick={() => {
                            if (!context.isSelected) {
                                // handleSelect would be called externally
                            }
                        }}
                    />
                )}
                ...
                <TextInput
                    ...
                    onSubmit={(val) => {
                        if (question.multiSelect) {
                            ...
                            if (fullAnswer) onAnswer(fullAnswer);
                        } else if (val.trim()) {
                            onAnswer(val.trim());
                        }
                    }}
...
function buildChoiceSelectionItems(
    questionOptions: Array<{ label: string; description: string }>,
    multiSelect: boolean,
    customOptionText: string
): Array<SelectionListItem<OptionItem>> {
    const list: Array<SelectionListItem<OptionItem>> = questionOptions.map((opt, i) => {
        const item: OptionItem = { key: `opt-${i}`, label: opt.label, description: opt.description, type: 'option', index: i };
        return { key: item.key, value: item };
    });
    if (multiSelect && questionOptions.length > 1) {
        list.push({ key: 'all', value: { key: 'all', label: 'All of the above', description: 'Select all options', type: 'all', index: list.length } });
    }
    list.push({ key: 'other', value: { key: 'other', label: customOptionText || '', description: '', type: 'other', index: list.length } });
    if (multiSelect) {
        list.push({ key: 'done', value: { key: 'done', label: 'Done', description: 'Finish selection', type: 'done', index: list.length }, hideNumber: true });
    }
    return list;
}
...
const ChoiceQuestionView: React.FC<ChoiceQuestionViewProps> = ({
    ...
    const horizontalPadding = String(numOptions).length + 2 + (question.multiSelect ? 4 : 1) + (question.multiSelect ? 0 : 2) + 1 + 2;
    ...
    const initialReducerState = useMemo((): ChoiceQuestionState => {
        if (!initialAnswer) return { selectedIndices: new Set<number>(), isCustomOptionSelected: false, isCustomOptionFocused: false };
        const selectedIndices = new Set<number>();
        let isCustomOptionSelected = false;
        const answers = question.multiSelect ? initialAnswer.split(', ') : [initialAnswer];
        answers.forEach((answer) => {
            const index = questionOptions.findIndex((opt) => opt.label === answer);
            if (index !== -1) selectedIndices.add(index);
            else isCustomOptionSelected = true;
        });
        return { selectedIndices, isCustomOptionSelected, isCustomOptionFocused: false };
    ...
    const initialCustomText = useMemo(() => {
        ...
        if (question.multiSelect) {
            ...
            return answers.find((a) => !questionOptions.some((opt) => opt.label === a)) || '';
        }
        return questionOptions.some((opt) => opt.label === initialAnswer) ? '' : initialAnswer;
    }, [initialAnswer, questionOptions, question.multiSelect]);

    const customBuffer = useTextBuffer({ initialText: initialCustomText, viewport: { width: Math.max(1, bufferWidth), height: 3 }, singleLine: false });
    ...
    const buildAnswerString = useCallback(
        (indices: Set<number>, includeCustomOption: boolean, customOption: string) => {
            ...
            questionOptions.forEach((opt, i) => { if (indices.has(i)) answers.push(opt.label); });
            if (includeCustomOption && customOption.trim()) {
                answers.push(expandPastePlaceholders(customOption, customBuffer.pastedContent).trim());
            }
            return answers.join(', ');
        },
    ...
    const handleExtraKeys = useCallback(
        (key: Key) => {
            if (isCustomOptionFocused && keyMatchers[Command.QUIT](key)) {
                if (customOptionText === '') return false;
                customBuffer.setText('');
                return true;
            }
            if (keyMatchers[Command.DIALOG_NAVIGATION_UP](key) || keyMatchers[Command.DIALOG_NAVIGATION_DOWN](key) || keyMatchers[Command.DIALOG_NEXT](key) || keyMatchers[Command.DIALOG_PREV](key) || keyMatchers[Command.MOVE_LEFT](key) || keyMatchers[Command.MOVE_RIGHT](key) || keyMatchers[Command.RETURN](key) || keyMatchers[Command.ESCAPE](key) || keyMatchers[Command.QUIT](key)) return false;
            if (/^[0-9]$/.test(key.sequence)) return false;
            const isPrintable = key.sequence && !key.ctrl && !key.alt && (key.sequence.length > 1 || key.sequence.charCodeAt(0) >= 32);
        ...
        [isCustomOptionFocused, customBuffer, onEditingCustomOption, customOptionText, keyMatchers]
    );

    useKeypress(handleExtraKeys, { isActive: true, priority: true });

    const selectionItems = useMemo(() => buildChoiceSelectionItems(questionOptions, !!question.multiSelect, customOptionText), [questionOptions, question.multiSelect, customOptionText]);

    const handleHighlight = useCallback((itemValue: OptionItem) => {
        const nowFocusingCustomOption = itemValue.type === 'other';
        dispatch({ type: 'SET_CUSTOM_FOCUSED', payload: { focused: nowFocusingCustomOption } });
        onEditingCustomOption?.(nowFocusingCustomOption);
    }, [onEditingCustomOption]);

    const handleSelect = useCallback((itemValue: OptionItem) => {
        if (question.multiSelect) {
            if (itemValue.type === 'option') dispatch({ type: 'TOGGLE_INDEX', payload: { index: itemValue.index, multiSelect: true } });
            else if (itemValue.type === 'other') dispatch({ type: 'TOGGLE_CUSTOM_SELECTED', payload: { multiSelect: true } });
            else if (itemValue.type === 'all') dispatch({ type: 'TOGGLE_ALL', payload: { totalOptions: questionOptions.length } });
            else if (itemValue.type === 'done') onAnswer(buildAnswerString(selectedIndices, isCustomOptionSelected, customOptionText));
        } else {
            if (itemValue.type === 'option') onAnswer(itemValue.label);
            else if (itemValue.type === 'other' && customOptionText.trim()) onAnswer(expandPastePlaceholders(customOptionText, customBuffer.pastedContent).trim());
        }
    }, [question.multiSelect, questionOptions.length, selectedIndices, isCustomOptionSelected, customOptionText, customBuffer.pastedContent, onAnswer, buildAnswerString]);

    useEffect(() => {
        if (customOptionText.trim() && !isCustomOptionSelected) dispatch({ type: 'SET_CUSTOM_SELECTED', payload: { selected: true, multiSelect: !!question.multiSelect } });
    }, [customOptionText, isCustomOptionSelected, question.multiSelect]);

    const { TITLE_MARGIN, questionHeightLimit, maxItemsToShow } = computeChoiceLayout(question, selectionItems, progressHeader, availableHeight, isAlternateBuffer);

    const renderItem = useCallback((item: SelectionListItem<OptionItem>, context: { isSelected: boolean; titleColor: string }) => {
        return renderChoiceItem(item.value, context, { selectedIndices, isCustomOptionSelected, questionOptions, question, customBuffer, buildAnswerString, onAnswer });
    }, [selectedIndices, isCustomOptionSelected, questionOptions, question, customBuffer, buildAnswerString, onAnswer]);

    return (
        <Box flexDirection="column">
            {progressHeader}
            <Box marginBottom={TITLE_MARGIN}>
                <MaxSizedBox maxHeight={questionHeightLimit} maxWidth={availableWidth} overflowDirection="bottom">
                    <Box flexDirection="column">
                        <MarkdownDisplay text={autoBoldIfPlain(question.question)} terminalWidth={availableWidth - DIALOG_PADDING} isPending={false} />
                        {question.multiSelect && <Text color={theme.text.secondary} italic>(Select all that apply)</Text>}
                    </Box>
                </MaxSizedBox>
            </Box>
            <BaseSelectionList<OptionItem> items={selectionItems} onSelect={handleSelect} onHighlight={handleHighlight} focusKey={isCustomOptionFocused ? 'other' : undefined} maxItemsToShow={maxItemsToShow} showScrollArrows={true} renderItem={renderItem} />
            {keyboardHints}
        </Box>
    );
};
...
export const AskUserDialog: React.FC<AskUserDialogProps> = ({
    ...
    return (
        <AskUserQuestionView
            ...
            answers={answers}
            isEditingCustomOption={isEditingCustomOption}
            width={width}
...
interface AskUserQuestionViewProps {
    ...
    questions: Question[];
    effectiveQuestion: Question | undefined;
    answers: { [key: string]: string };
    isEditingCustomOption: boolean;
    width: number;
    availableHeight: number | undefined;
    ...
    onAnswer: (answer: string) => void;
    onSelectionChange: (answer: string) => void;
    onEditingCustomOption: (isEditing: boolean) => void;
}

const AskUserQuestionView: React.FC<AskUserQuestionViewProps> = ({
    currentQuestion,
