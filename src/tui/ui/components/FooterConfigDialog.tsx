...
const buildPreviewContent = (
    ...
    const getColor = (_id: string, defaultColor?: string) => defaultColor || itemColor;
    const mockData: Record<string, React.ReactNode> = {
        ...
        'model-name': <Text color={getColor('model-name', itemColor)}>gemini-2.5-pro</Text>,
        ...
        'token-count': <Text color={getColor('token-count', itemColor)}>1.5k tokens</Text>
    };
...
const makeRenderFooterConfigItem = (
    selectedIds: Set<string>,
    showLabels: boolean
) => (item: SelectionListItem<FooterConfigItem>, { isSelected, titleColor }: { isSelected: boolean; titleColor: string }) => {
...
export const FooterConfigDialog: React.FC<FooterConfigDialogProps> = ({
    ...
    const previewContent = useMemo(
        () => buildPreviewContent(focusKey, orderedIds, selectedIds, showLabels),
        ...
        Select which items to display in the footer.
            ...
            <Box flexDirection="column" marginTop={1} flexGrow={1}>
                <BaseSelectionList<FooterConfigItem>
                    ...
                    selectedIndicator=">"
                    renderItem={makeRenderFooterConfigItem(selectedIds, showLabels)}
                />
            </Box>
