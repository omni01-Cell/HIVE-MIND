...
const buildStatsRows = (
    activeModels: Array<[string, ModelMetrics]>,
    hasCached: boolean,
    hasThoughts: boolean,
    hasTool: boolean,
    allRoles: LlmRole[]
    ...
    rows.push(createRow('Errors', (m) => {
        ...
        return <Text color={m.api.totalErrors > 0 ? theme.status.error : theme.text.primary}>{m.api.totalErrors.toLocaleString()} ({errorRate.toFixed(1)}%)</Text>;
    }));
    ...
    rows.push(createRow('Input', (m) => <Text color={theme.text.primary}>{m.tokens.input.toLocaleString()}</Text>, { isSubtle: true }));
    if (hasCached) {
        rows.push(createRow('Cache Reads', (m) => {
            const cacheHitRate = calculateCacheHitRate(m);
            return <Text color={theme.text.secondary}>{m.tokens.cached.toLocaleString()} ({cacheHitRate.toFixed(1)}%)</Text>;
        }, { isSubtle: true }));
    }
    if (hasThoughts) {
        rows.push(createRow('Thoughts', (m) => <Text color={theme.text.primary}>{m.tokens.thoughts.toLocaleString()}</Text>, { isSubtle: true }));
    }
    if (hasTool) {
    ...
    if (allRoles.length > 0) {
        ...
        allRoles.forEach((role) => {
            rows.push({ metric: role, isSection: true, color: theme.text.primary });
            const addRoleMetric = (metric: string, getValue: (r: RoleMetrics) => string | React.ReactNode) => {
                ...
                activeModels.forEach(([name, metrics]) => {
                    ...
                    row[name] = roleMetrics ? getValue(roleMetrics) : <Text color={theme.text.secondary}>-</Text>;
                });
                rows.push(row);
            };
