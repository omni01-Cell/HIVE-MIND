...
function handleSearchKey(key: { name?: string; sequence?: string }, state: SessionBrowserState): boolean {
    if (key.name === 'escape') {
    ...
    if (key.name === 'enter') {
        ...
        if (selectedSession && !selectedSession.isCurrentSession) return true;
        return true;
    }
    if (key.sequence && key.sequence.length === 1 && !key.alt && !key.ctrl && !key.cmd) {
...
function handleNavigationKey(
    ...
    if (key.sequence === 'g') { state.setActiveIndex(0); state.setScrollOffset(0); return true; }
    if (key.sequence === 'G') { state.setActiveIndex(state.totalSessions - 1); state.setScrollOffset(Math.max(0, state.totalSessions - SESSIONS_PER_PAGE)); return true; }
    if (key.sequence === 's') { cycleSortOrder(); return true; }
    if (key.sequence === 'r') { state.setSortReverse(!state.sortReverse); return true; }
    if (key.sequence === '/') { state.setIsSearchMode(true); return true; }
    if (key.sequence === 'q' || key.sequence === 'Q' || key.name === 'escape') { onExit(); return true; }
    if (key.sequence === 'x' || key.sequence === 'X') {
        const selectedSession = state.filteredAndSortedSessions[state.activeIndex];
        if (selectedSession && !selectedSession.isCurrentSession) {
            onDeleteSession(selectedSession).then(() => {
    ...
    if (key.sequence === 'u') { moveSelection(-Math.round(SESSIONS_PER_PAGE / 2)); return true; }
    if (key.sequence === 'd') { moveSelection(Math.round(SESSIONS_PER_PAGE / 2)); return true; }
    if (key.name === 'enter' && state.filteredAndSortedSessions[state.activeIndex]) {
        const selectedSession = state.filteredAndSortedSessions[state.activeIndex];
        if (!selectedSession.isCurrentSession) onResumeSession(selectedSession);
        return true;
    }
    if (key.name === 'up') { moveSelection(-1); return true; }
    if (key.name === 'down') { moveSelection(1); return true; }
    if (key.name === 'pageup') { moveSelection(-SESSIONS_PER_PAGE); return true; }
    if (key.name === 'pagedown') { moveSelection(SESSIONS_PER_PAGE); return true; }
    return false;
}
...
export const useSessionBrowserInput = (
    ...
    useKeypress(
        (key) => state.isSearchMode
            ? handleSearchKey(key, state)
            : handleNavigationKey(key, state, cycleSortOrder, onResumeSession, onDeleteSession, onExit, moveSelection),
        { isActive: true }
    );
};
