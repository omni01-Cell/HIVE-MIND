...
export const TableRenderer: React.FC<TableRendererProps> = ({
    ...
    const { wrappedHeaders, wrappedRows, adjustedWidths } = useMemo(() => {
        ...
        const finalContentWidths = allocateColumnWidths(constraints, availableWidth);
        ...
        const adjustedWrappedHeaders = wrapAndProcessRow(styledHeaders);
        const adjustedWrappedRows = styledRows.map((row) => wrapAndProcessRow(row));

        // Use the TIGHTEST widths that fit the wrapped content + padding
        const finalWidths = actualColumnWidths.map((w) => w + COLUMN_PADDING);

        return { wrappedHeaders: adjustedWrappedHeaders, wrappedRows: adjustedWrappedRows, adjustedWidths: finalWidths };
