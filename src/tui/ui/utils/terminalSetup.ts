...
async function configureVSCodeStyle(
    ...
    try {
        ...
        const results = targetBindings.map((target) => {
            const hasExistingBinding = keybindings.some((kb) => {
                if (!isKeybinding(kb)) return false;
                return (
                    kb.key === target.key &&
        ...
        for (const { hasExistingBinding, target } of results) {
...
export const TERMINAL_SETUP_CONSENT_MESSAGE =
  'HIVE-MIND TUI works best with Shift+Enter/Ctrl+Enter for multiline input. ' +
