...
function sortSuggestionsByRelevance(suggestions: SlashCommand[], lowerPartial: string): SlashCommand[] {
    return [...suggestions].sort((a, b) => {
        const aNameExact = a.name.toLowerCase() === lowerPartial;
        const bNameExact = b.name.toLowerCase() === lowerPartial;
        if (aNameExact && !bNameExact) return -1;
        if (!aNameExact && bNameExact) return 1;
        const aAltExact = a.altNames?.some((alt) => alt.toLowerCase() === lowerPartial) || false;
        const bAltExact = b.altNames?.some((alt) => alt.toLowerCase() === lowerPartial) || false;
        ...
        if (!aNamePrefix && bNamePrefix) return 1;
        const aAltPrefix = a.altNames?.some((alt) => alt.toLowerCase().startsWith(lowerPartial)) || false;
        const bAltPrefix = b.altNames?.some((alt) => alt.toLowerCase().startsWith(lowerPartial)) || false;
