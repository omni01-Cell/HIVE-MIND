...
function buildFallbackMessage(
...
): { message: string; isTerminalQuotaError: boolean; isModelNotFoundError: boolean } {
    let message: string;
    ...
    if (error instanceof TerminalQuotaError) {
        ...
        const messageLines = [
            ...
            authType === AuthType.LOGIN_WITH_GOOGLE ? '/auth to switch to API key.' : null
        ].filter(Boolean);
        ...
        if (VALID_GEMINI_MODELS.has(failedModel)) {
            ...
            message = [`Model "${failedModel}" was not found or is invalid.`, '/model to switch models.'].join('\n');
        }
    } else {
        message = ['We are currently experiencing high demand.', 'We apologize and appreciate your patience.', '/model to switch models.'].join('\n');
    }

    return { message, isTerminalQuotaError, isModelNotFoundError };
}
...
export function useQuotaAndFallback({
    ...
    useEffect(() => {
        const fallbackHandler: FallbackModelHandler = async (
            ...
            const { message, isTerminalQuotaError, isModelNotFoundError } = buildFallbackMessage(
                error, failedModel, usageLimitReachedModel, contentGeneratorConfig?.authType
            );

            if (isTerminalQuotaError) {
                const isInsufficientCredits = (error as TerminalQuotaError).isInsufficientCredits;
                if (
                    !isInsufficientCredits &&
                    paidTier?.availableCredits &&
                    isOverageEligibleModel(failedModel)
                ) {
                    const resetTime = (error as TerminalQuotaError).retryDelayMs
                        ? getResetTimeMessage((error as TerminalQuotaError).retryDelayMs!)
                        : undefined;
                    const overageStrategy = config.getBillingSettings().overageStrategy;
                    const creditsResult = await handleCreditsFlow({
                        config, paidTier, overageStrategy, failedModel, fallbackModel,
                        usageLimitReachedModel, resetTime, historyManager,
                        setModelSwitchedFromQuotaError, isDialogPending,
                        setOverageMenuRequest, setEmptyWalletRequest
                    });
