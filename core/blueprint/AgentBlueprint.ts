...
class BlueprintManager {
    ...
    public loadBlueprint(blueprintId: string): AgentBlueprint {
        ...
        const path = join(process.cwd(), 'config', 'blueprints', `${blueprintId}.json`);
        if (!existsSync(path)) {
            throw new Error(`[BlueprintManager] Blueprint file not found on disk: config/blueprints/${blueprintId}.json`);
        }
