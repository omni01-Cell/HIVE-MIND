...
export class GeminiLiveProvider {
     ...
     * @param {Function} callback - (toolCall) => Promise<result>
     */
    setToolCallback(callback) {
     ...
     * @private
     */
    async _waitForResponse(queue, session) {
        const audioParts = [];
        let transcription = '';
        let emotion = null;
        let done = false;
        ...
        return {
            ...
            audioBuffer: audioParts.length > 0
                ? Buffer.concat(audioParts.map(d => Buffer.from(d, 'base64')))
                : null,
     ...
     * @private
     */
    _createWavBuffer(pcmParts, sampleRate = 24000) {
        const numChannels = 1;
        const bitsPerSample = 16;
        const byteRate = sampleRate * numChannels * bitsPerSample / 8;
        const blockAlign = numChannels * bitsPerSample / 8;

        const pcmBuffer = Buffer.concat(pcmParts.map(d => Buffer.from(d, 'base64')));
