...
- ✅ Done: Session 16 — Branchement du Navigateur d'Historique sur Supabase (synchronisation asynchrone des sessions TUI sous context_id dynamique). Modification de `tui-local` vers les IDs de session dynamiques dans `HiveTransport.ts` et `hiveCommands.ts`, et résolution des rejets de promesses "fire-and-forget" dans `hiveConfig.ts` via des wrappers IIFE asynchrones. Réalisé par Jules au nom de JULES AI.
