...
- ✅ Done (by Jules AI): Implemented missing `deleteSession` and `deleteCurrentSessionAsync` in `src/tui/config/hiveConfig.ts` and updated `ChatRecordingService` interface in `src/tui/ui/hooks/useHistoryManager.ts` to fix deletion errors in the UI.
