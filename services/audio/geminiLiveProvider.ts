...
export class GeminiLiveProvider {
     ...
     */
    async connect(sessionConfig: any = {}) {
    ...
    _sendSetup(config: any) {
        ...
        if (config.tools && config.tools.length > 0) {
            setupMessage.setup.tools = [{
                function_declarations: config.tools.map((tool: any) => ({
     ...
     */
    async waitForResponse(timeoutMs: any = 30000) {
        return new Promise((resolve: any, reject: any) => {
     ...
     */
    async processAudioWithTools(options: any) {
