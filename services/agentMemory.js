...
export const agentMemory = {
     ...
     */
    async logAction(chatId, toolName, params, result, status, errorMessage = null) {
        try {
     ...
     */
    async getRecentActions(chatId, limit = 5) {
        try {
     ...
     */
    async hasRecentFailure(chatId, toolName, withinMinutes = 30) {
        try {
     ...
     * @returns {Promise<Array>}
     */
    async getLessonsLearned(chatId, limit = 3) {
        try {
            const { data, error } = await supabase
                .from('agent_actions')
     ...
     * @returns {Promise<Array>}
     */
    async getGlobalLessonsLearned(limit = 10) {
        try {
            const { data, error } = await supabase
                .from('agent_actions')
                .select('tool_name, error_message, created_at')
