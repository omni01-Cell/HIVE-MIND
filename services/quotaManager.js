...
class QuotaManager {
     ...
     */
    async isModelAvailable(modelId, estimatedCost = 0) {
     ...
     */
    async recordQuotaExceeded(modelId, timeoutSeconds = 60) {
    ...
    async filterAvailableProviders(candidates) {
        return candidates;
    }
     ...
     */
    async getModelHealth(modelId, margins = { rpm: 0.20, tpm: 0.10, rpd: 0.05 }) {
        const result = {
            healthy: true,
            blocked: false,
            rpmUsed: 0,
     ...
     */
    async filterHealthyModels(modelIds, margins = { rpm: 0.20, tpm: 0.10, rpd: 0.05 }) {
     ...
     */
    async getHealthyFamilies(familiesConfig, margins = { rpm: 0.20, tpm: 0.10, rpd: 0.05 }) {
        ...
        for (const [familyName, familyConfig] of Object.entries(familiesConfig)) {
            ...
            const chatModels = models
                .filter(m => !m.id?.includes('embedding') && !m.types?.includes('embedding'))
                .map(m => m.id);
     ...
     * @private
     */
    _allowWithLocalRateLimit(modelId) {
