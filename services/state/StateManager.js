...
export const StateManager = {
     ...
     */
    async updateUserInteraction(identifier, pushName) {
        const jid = await IdentityMap.resolve(identifier);
        const cacheKey = `user:${jid}:data`;
     ...
     */
    async processSyncQueue(batchSize = 50) {
     ...
     */
    async recordGroupActivity(groupJid, userJid) {
     ...
     */
    async getGroupLeaderboard(groupJid, limit = 10) {
        const key = `group:${groupJid}:leaderboard`;
     ...
     */
    async getUserGroupScore(groupJid, userJid) {
        const key = `group:${groupJid}:leaderboard`;
