...
export class EmbeddingSyncService {
     ...
     */
    async updateEmbeddingsBatch(tools: any) {
        ...
        for (let i = 0; i < tools.length; i += this.batchSize) {
            ...
            await Promise.all(batch.map(async (tool: any) => {
                try {
     ...
     */
    async updateToolEmbedding(toolName: any, embedding: any) {
     ...
     * @param {string} toolName - Nom de l'outil
     */
    async removeToolEmbedding(toolName: any) {
        const { error } = await supabase
            .from('bot_tools')
     ...
     */
    async getStoredDefinitionHash(toolName: any) {
     ...
     */
    async saveDefinitionHash(toolName: any, hash: any) {
        const { error } = await supabase
            .from('tool_metadata')
            .upsert({
                tool_name: toolName,
